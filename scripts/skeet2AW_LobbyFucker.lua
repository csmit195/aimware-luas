...
local function getMessageToSend()
    ...
    local target = '(LobbyAPI.BIsHost()) ? PartyListAPI.GetXuidByIndex(' .. (targetSlider:GetValue() - 1) .. ') : MyPersonaAPI.GetXuid()' --LobbyAPI.BIsHost()) ? PartyListAPI.GetXuidByIndex(1) : MyPersonaAPI.GetXuid()
    local extraMessage = ""
